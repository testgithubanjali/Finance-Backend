...
func CreateRecord(record models.Record) error {
	log.Printf("RecordRepo: CreateRecord user_id=%s id=%s amount=%.2f type=%s",
		record.UserID, record.ID, record.Amount, record.Type)
	...
	if err != nil {
		log.Printf("RecordRepo: CreateRecord failed user_id=%s id=%s error=%v",
			record.UserID, record.ID, err)
		return err
	}

	log.Printf("RecordRepo: CreateRecord succeeded user_id=%s id=%s",
...
func GetRecordsByUser(userID string) ([]models.Record, error) {
	query := `SELECT id, user_id, amount, type, category, notes 
	...
	for rows.Next() {
		var r models.Record
		if err := rows.Scan(&r.ID, &r.UserID, &r.Amount, &r.Type, &r.Category, &r.Notes); err != nil {
			log.Printf("RecordRepo: scan error: %v", err)
			return nil, err
		}
